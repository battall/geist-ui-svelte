...
fetch("https://vercel.com/design/icons")
  ...
  .then(() => {
    ...
    console.log("Done removing old icons");
  })
  .then(() => {
    ...
    Array.from(icons).forEach(async (icon) => {
      ...
      fs.writeFileSync(outputPath + componentName + ".svelte", component);
    });
...
const parseSvg = (svg, styles) => {
  ...
  // Add props that passed from parent
  svg = svg.replace(
    /<svg([^>]+)>/,
    '<svg$1 {...$$$restProps} height={size} width={size} style="color: {color};">'
  );
